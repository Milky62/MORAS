class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            ...
            if len(l) == 2:
                ...
                self._flag = False
                Parser._error("Parser", n, "Invalid goto syntax.");
                return ""
            ...
            if len(l) == 2:
                ...
                self._flag = False
                Parser._error("Parser", n, "Invalid if-goto syntax.");
                return ""
    ...
    def _push(self, src, loc, n):
        ...
        if not loc.isdigit() or int(loc) < 0:
            self._flag = False
            Parser._error("Push", n, "Invalid location '{loc}'. Must be a non-negative integer.")
            return ""
    ...
    def _pop(self, dst, loc, n):
        ...
        if not loc.isdigit() or int(loc) < 0:
            self._flag = False
            Parser._error("Pop", n, "Invalid location '{loc}'. Must be a non-negative integer.")
            return ""
        ...
        if dst == "local":
            ...
            self._flag = False
            Parser._error("Pop", n, "Undefined destination \"" + dst + "\".");
            return ""
        return l
    

    # b)
    def _goto(self, label, n):
